...
import type { CreateTimeLogInput, Task, Project, WorkspaceUser } from '@/services/NotionService'

interface AddTimeLogModalProps {
  open: boolean
  onClose: () => void
  onSubmit: (data: CreateTimeLogInput) => Promise<unknown>
  tasks: Task[]
  projects: Project[]
  workspaceUsers: WorkspaceUser[]
}

export function AddTimeLogModal({ open, onClose, onSubmit, tasks, projects, workspaceUsers }: AddTimeLogModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-white/[0.12] bg-[#111111] p-6 shadow-[0_0_80px_rgba(255,255,255,0.04)] animate-fade-up">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            <label className="mb-2 block text-[11px] font-medium uppercase tracking-[0.1em] text-white/40">Description</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="mb-2 block text-[11px] font-medium uppercase tracking-[0.1em] text-white/40">Person</label>
            <select
              value={personId}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-2 block text-[11px] font-medium uppercase tracking-[0.1em] text-white/40">Hours</label>
              <input
                type="number"
                step="0.25"
                min="0"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-2 block text-[11px] font-medium uppercase tracking-[0.1em] text-white/40">Task</label>
              <select
                value={taskId}
          ...
          <div>
            <label className="mb-2 block text-[11px] font-medium uppercase tracking-[0.1em] text-white/40">Notes</label>
            <input
              type="text"
              value={notes}
          ...
          <div className="flex items-center justify-end gap-3 pt-3">
            ...
            <button type="submit" disabled={!name.trim() || !hours || submitting} className="h-10 rounded-xl bg-white px-5 text-sm font-semibold text-black transition-all hover:shadow-[0_0_30px_rgba(255,255,255,0.2)] active:scale-[0.97] disabled:opacity-30">
