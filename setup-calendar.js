...
async function setup() {
  // Step 1: Remove "In Meeting" from team statuses — update existing members
  console.log('Updating team members with "In Meeting" status to "Available"...')
  const teamDS = process.env.NOTION_TEAM_DATA_SOURCE_ID
  const teamResults = await notion.dataSources.query({ data_source_id: teamDS })
  for (const page of teamResults.results) {
    if (page.properties?.Status?.select?.name === 'In Meeting') {
      await notion.pages.update({
        page_id: page.id,
        properties: { Status: { select: { name: 'Available' } } },
      })
      console.log(`  Updated: ${page.properties.Name?.title?.[0]?.plain_text}`)
    }
  }

  // Step 2: Create Calendar/Leave database
  console.log('\nCreating Leave Records database...')
  const db = await notion.databases.create({
    parent: { type: 'page_id', page_id: PARENT_PAGE_ID },
    title: [{ type: 'text', text: { content: 'Leave Records' } }],
  })
  ...
  // Step 3: Add sample leave records
  console.log('\nAdding sample leave records...')
